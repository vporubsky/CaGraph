...
for treatment_group_index in [0, 1, 2]:
    for mouse_id_index in range(len(all_files[treatment_group_index])):
        ...
        try:
            con_A_sigma_val = nn.get_smallworld_largest_subnetwork(graph=conA, threshold=threshold)
            print(f'Completed analysis of {mouse_id} context A with sigma val: {con_A_sigma_val}')
        except:
            con_A_sigma_val = float('nan')
            print(f'Completed analysis of {mouse_id} context A but sigma val could not be computed.')
        try:
            con_B_sigma_val = nn.get_smallworld_largest_subnetwork(graph=conB, threshold=threshold)
            print(f'Completed analysis of {mouse_id} context B with sigma val: {con_B_sigma_val}')
        except:
            con_B_sigma_val = float('nan')
            print(f'Completed analysis of {mouse_id} context B but sigma val could not be computed.')

        if treatment_group_index == 0:
            con_A_sw_D1.append(con_A_sigma_val)
            con_B_sw_D1.append(con_B_sigma_val)

        elif treatment_group_index == 1:
            con_A_sw_D5.append(con_A_sigma_val)
            con_B_sw_D5.append(con_B_sigma_val)

        elif treatment_group_index == 2:
            con_A_sw_D9.append(con_A_sigma_val)
            con_B_sw_D9.append(con_B_sigma_val)
...
plt.ylabel('Smallworld sigma val')
plt.title('Edge weight threshold: ' + str(threshold))
plt.show()

data.to_csv(path_or_buf=f'smallworld_sigma_vals_threshold_{threshold}.csv')

# To reload from saved dataframe:
data = pd.read_csv('smallworld_sigma_vals_threshold_0.5.csv', index_col=0)