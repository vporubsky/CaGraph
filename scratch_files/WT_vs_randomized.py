...
for day in [0, 1, 2, 3]:
    for mouse_id_index in range(len(all_WT_files[day])):
        ...
        if day == 0:
            ...
            cc_A_rand = nn.get_clustering_coefficient(graph=nn.get_random_graph(threshold=0.3))
            cc_B_rand = nn.get_clustering_coefficient(graph=nn.get_random_graph(threshold=0.3))
        else:
            cc_A = nn.get_context_A_clustering_coefficient()
            cc_B = nn.get_context_B_clustering_coefficient()
            cc_A_rand = nn.get_clustering_coefficient(graph=nn.get_random_context_A_graph())
            cc_B_rand = nn.get_clustering_coefficient(graph=nn.get_random_context_B_graph())


        if day == 0:
            con_A_cc_D0.append(cc_A)
            con_B_cc_D0.append(cc_B)
            con_A_cc_D0_rand.append(cc_A_rand)
            con_B_cc_D0_rand.append(cc_B_rand)

        elif day == 1:
            con_A_cc_D1.append(cc_A)
            con_B_cc_D1.append(cc_B)
            con_A_cc_D1_rand.append(cc_A_rand)
            con_B_cc_D1_rand.append(cc_B_rand)

        elif day == 2:
            con_A_cc_D5.append(cc_A)
            con_B_cc_D5.append(cc_B)
            con_A_cc_D5_rand.append(cc_A_rand)
            con_B_cc_D5_rand.append(cc_B_rand)

        elif day == 3:
            con_A_cc_D9.append(cc_A)
            con_B_cc_D9.append(cc_B)
            con_A_cc_D9_rand.append(cc_A_rand)
            con_B_cc_D9_rand.append(cc_B_rand)
...
#%%
con_A_cc_D0_rand = [item for sublist in con_A_cc_D0_rand for item in sublist]
con_B_cc_D0_rand = [item for sublist in con_B_cc_D0_rand for item in sublist]

con_A_cc_D1_rand = [item for sublist in con_A_cc_D1_rand for item in sublist]
con_B_cc_D1_rand = [item for sublist in con_B_cc_D1_rand for item in sublist]

con_A_cc_D5_rand = [item for sublist in con_A_cc_D5_rand for item in sublist]
con_B_cc_D5_rand = [item for sublist in con_B_cc_D5_rand for item in sublist]

con_A_cc_D9_rand = [item for sublist in con_A_cc_D9_rand for item in sublist]
con_B_cc_D9_rand = [item for sublist in con_B_cc_D9_rand for item in sublist]
...
plt.subplot(121)
nn.plot_CDF_compare_two_samples(data_list=[con_A_cc_D1_rand, con_A_cc_D1], color_list=['lightgrey', 'mistyrose'])
plt.legend(['Random D1 context A', 'D1 context A'])
plt.xlabel('Clustering coefficient')

plt.subplot(122)
nn.plot_CDF_compare_two_samples(data_list=[con_B_cc_D1_rand, con_B_cc_D1], color_list=['lightgrey', 'turquoise'])
plt.legend(['Random D1 context B', 'D1 context B'])
plt.xlabel('Clustering coefficient')

plt.savefig(export_path + 'randomized_D1_WT.png', dpi=200)
plt.show()
#%%
plt.subplot(132)
