...
#%% Attempt new dataframe to compute average and standard deviation
labels = ['D1_A', 'D1_B', 'D9_A', 'D9_B']
raw = [con_A_num_hubs_D1, con_B_num_hubs_D1, con_A_num_hubs_D9, con_B_num_hubs_D9]
average = [np.average(con_A_num_hubs_D1), np.average(con_B_num_hubs_D1), np.average(con_A_num_hubs_D9), np.average(con_B_num_hubs_D9)]
stdev = [np.std(con_A_num_hubs_D1), np.std(con_B_num_hubs_D1), np.std(con_A_num_hubs_D9), np.std(con_B_num_hubs_D9)]

# plt.plot(labels, [con_A_num_hubs_D1, con_B_num_hubs_D1, con_A_num_hubs_D9, con_B_num_hubs_D9], 'o')
# plt.errorbar(labels, average, stdev, ecolor = 'k', linestyle='None',capsize=5, marker='s', markeredgecolor ='k', markerfacecolor = 'k', alpha=  0.5)
# plt.ylabel('Hub count/ neuron count')

#%% plot with seaborn
plt.figure(1)

data = pd.DataFrame(np.transpose(np.array(raw)), columns=labels, index=mouse_id_indices)
sns.boxplot(data=data, whis = 10);
sns.swarmplot(data=data, color = 'k');

y, h, col = data['D9_B'].max() + 0.02, 0.02, 'k'
x1, x2 = 2, 3  # columns to match
plt.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1.5, c=col)
plt.text((x1+x2)*.5, y+h, "*", ha='center', va='bottom', color=col)
plt.text((x1+x2)*.5, y+h+0.015, "p = 0.024", ha='center', va='bottom', color=col)
...
# plt.text((x1+x2)*.5, y+h, "*", ha='center', va='bottom', color=col)
# plt.text((x1+x2)*.5, y+h+0.015, "p = 0.024", ha='center', va='bottom', color=col)
#plt.savefig('tmp_fig.png', dpi = 300)
